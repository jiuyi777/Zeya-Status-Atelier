...
export function applyStatusBeautyMobileLayout(regexScript, rule) {
    if (rule?.structure === 'moon-collage') {
        ...
        const title = escapeHtml(rule?.title || BUNDLED_DEFAULT_TITLES.get(rule.structure) || '月下蝶影');
        const mobile = `<section class="sta-moon-mobile" ${marker} aria-label="月下蝶影手机布局"><header class="sta-moon-mobile-head"><div class="sta-moon-mobile-photo"><img data-st-avatar alt="当前角色头像"></div><div><small>STATUS ATELIER · MOON 03</small><h1 data-design-title>${title}</h1><p>月色、蝶影与此刻未说出口的话</p></div></header><div class="sta-moon-mobile-meters"><article><span>${label(0)}</span><strong data-capture="1">$1</strong></article><article><span>${label(1)}</span><strong data-capture="2">$2</strong></article></div><div class="sta-moon-mobile-records"><article><span>${label(2)}</span><p data-capture="3">$3</p></article><article><span>${label(3)}</span><p data-capture="4">$4</p></article><article><span>${label(4)}</span><p data-capture="5">$5</p></article><article><span>${label(5)}</span><p data-capture="6">$6</p></article></div><footer class="sta-moon-mobile-notes"><article><span>${label(6)}</span><p data-capture="7">$7</p></article><article><span>${label(7)}</span><p data-capture="8">$8</p></article></footer></section>`;
        const style = `<style ${marker}>.sta-moon-mobile{display:none}@media(max-width:700px){html,body{min-height:0!important;background:transparent!important}.design-page{display:block!important;padding:0!important;background:transparent!important}.moon-art{width:100%!important;height:auto!important;overflow:hidden!important;border:1px solid #7782a5!important;border-radius:18px!important;background:#edf0f5!important;box-shadow:0 10px 28px rgba(32,40,67,.2)!important}.moon-art[open]>.art-stage{display:none!important}.moon-art[open]>.sta-moon-mobile{display:block!important}.moon-art>summary{right:11px!important;top:11px!important;width:30px!important;height:30px!important;color:#f8f1e5!important;background:#39415f!important;border-color:#fff9!important}.sta-moon-mobile{position:relative;overflow:hidden;padding:13px;color:#303751;background:radial-gradient(circle at 90% 8%,rgba(157,167,199,.28),transparent 31%),linear-gradient(150deg,#f5f1e8 0%,#e8edf3 52%,#eee5df 100%);font:400 clamp(14px,3.9vw,17px)/1.62 "Noto Serif SC","Source Han Serif SC","Songti SC",serif}.sta-moon-mobile:before{content:"";position:absolute;right:-38px;bottom:105px;width:132px;height:132px;border:22px solid rgba(81,91,129,.08);border-radius:50%;pointer-events:none}.sta-moon-mobile>*{position:relative}.sta-moon-mobile-head{display:grid;grid-template-columns:105px minmax(0,1fr);gap:13px;align-items:end;padding:2px 38px 14px 2px;border-bottom:3px double #6d7595}.sta-moon-mobile-photo{height:142px;overflow:hidden;border:7px solid #f7f0e4;border-radius:48px 48px 10px 10px;background:#cfd5df;box-shadow:0 7px 18px rgba(47,54,83,.24),inset 0 0 0 1px #79829f}.sta-moon-mobile-photo img{display:block;width:100%;height:100%;object-fit:cover;object-position:center 18%}.sta-moon-mobile-head small{display:block;color:#7d6680;font:700 9px/1.3 Georgia,serif;letter-spacing:.13em}.sta-moon-mobile-head h1{margin:5px 0 4px;color:#343b59;font-size:clamp(27px,8vw,36px);font-weight:650;line-height:1.08;letter-spacing:.08em}.sta-moon-mobile-head p{margin:0;color:#736d7c;font-size:.82em}.sta-moon-mobile-meters{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin-top:11px}.sta-moon-mobile-meters article{min-width:0;padding:10px 11px;border:1px solid rgba(69,79,113,.3);border-top:4px solid #485271;background:rgba(255,252,246,.72)}.sta-moon-mobile span{display:block;color:#745d78;font-size:.78em;font-weight:650;letter-spacing:.12em}.sta-moon-mobile-meters strong{display:block;margin-top:4px;color:#303956;font-size:1.1em;font-weight:600;line-height:1.4;overflow-wrap:anywhere}.sta-moon-mobile-records{display:grid;gap:8px;margin-top:9px}.sta-moon-mobile-records article{min-width:0;padding:11px 12px;border-left:4px solid #7a6580;background:rgba(255,253,248,.74);box-shadow:0 4px 12px rgba(48,57,86,.08)}.sta-moon-mobile-records article:nth-child(even){border-left-color:#667995;background:rgba(237,241,246,.78)}.sta-moon-mobile p{margin:5px 0 0;font-weight:400;line-height:1.65;white-space:normal;overflow-wrap:anywhere}.sta-moon-mobile-notes{display:grid;grid-template-columns:minmax(0,1fr);gap:8px;margin-top:9px;padding-top:9px;border-top:1px dashed #8b8293}.sta-moon-mobile-notes article{padding:10px 12px;border-radius:12px;background:linear-gradient(135deg,rgba(67,76,108,.92),rgba(111,91,112,.9));color:#fff8ed}.sta-moon-mobile-notes span{color:#e8d7c6}.sta-moon-mobile-notes p{color:#fffaf2}}@media(max-width:390px){.sta-moon-mobile{padding:11px}.sta-moon-mobile-head{grid-template-columns:92px minmax(0,1fr);gap:10px;padding-right:35px}.sta-moon-mobile-photo{height:126px}.sta-moon-mobile-meters{grid-template-columns:minmax(0,1fr)}.sta-moon-mobile-head h1{letter-spacing:.04em}}</style>`;
    ...
    if (['beauty-clock-travel-11', 'beauty-flower-reader-12', 'beauty-olive-ticket-13', 'beauty-cat-rabbit-14', 'beauty-rabbit-track-15'].includes(rule?.structure)) {
        const marker = 'data-status-atelier-layered-mobile';
        const replacement = String(regexScript?.replaceString || '');
        if (!replacement || replacement.includes(marker)) return regexScript;
        const defaults = BUNDLED_DEFAULT_FIELDS.get(rule.structure) || [];
        const fields = rule?.pages?.[0]?.fields || rule?.pageFields || [];
        ...
        const label = index => escapeHtml(fields[index]?.label || defaults[index]?.[0] || `字段${index + 1}`);
        const skins = {
            'beauty-clock-travel-11': { number: '11', overline: 'CLOCKWORK TRAVEL PAGE', glyph: '◷', accent: '#9a623d', deep: '#344b49', paper: '#f2e4cb', wash: '#d6ded4' },
            'beauty-flower-reader-12': { number: '12', overline: 'FLOWER READER NOTE', glyph: '❀', accent: '#8e667b', deep: '#475f55', paper: '#f2e8df', wash: '#d9e0d5' },
            'beauty-olive-ticket-13': { number: '13', overline: 'OLIVE TICKET BOOK', glyph: '⌑', accent: '#8a6c3b', deep: '#465540', paper: '#eee6cf', wash: '#d8dcc0' },
            'beauty-cat-rabbit-14': { number: '14', overline: 'CAT & RABBIT NIGHT TALK', glyph: '☾', accent: '#a45f78', deep: '#34354f', paper: '#f0e2e5', wash: '#d9d7e6' },
            'beauty-rabbit-track-15': { number: '15', overline: 'RABBIT PLAN TRACK', glyph: '↟', accent: '#a94e4b', deep: '#4b3e3b', paper: '#f2e7d7', wash: '#dfd2c0' },
        };
        const skin = skins[rule.structure];
        const title = escapeHtml(rule?.title || BUNDLED_DEFAULT_TITLES.get(rule.structure) || '人物状态');
        const variables = `--sta-accent:${skin.accent};--sta-deep:${skin.deep};--sta-paper:${skin.paper};--sta-wash:${skin.wash}`;
        const mobile = `<section class="sta-layered-mobile sta-layered-${skin.number}" ${marker} style="${variables}" aria-label="${title}手机布局"><header class="sta-layered-mobile-head"><div class="sta-layered-mobile-avatar"><img alt="当前角色头像"></div><div><small>${skin.overline}</small><h1 data-design-title>${title}</h1><p>${skin.glyph} · STATUS ATELIER ${skin.number}</p></div></header><div class="sta-layered-mobile-meters"><article><span>${label(0)}</span><strong data-capture="1">$1</strong></article><article><span>${label(1)}</span><strong data-capture="2">$2</strong></article></div><div class="sta-layered-mobile-primary"><article><span>${label(2)}</span><p data-capture="3">$3</p></article><article><span>${label(3)}</span><p data-capture="4">$4</p></article><article><span>${label(4)}</span><p data-capture="5">$5</p></article><article class="is-letter"><span>${label(5)}</span><p data-capture="6">$6</p></article></div><div class="sta-layered-mobile-body"><article><span>${label(6)}</span><p data-capture="7">$7</p></article><article><span>${label(7)}</span><p data-capture="8">$8</p></article><article><span>${label(8)}</span><p data-capture="9">$9</p></article></div><details class="sta-layered-mobile-more"><summary>展开思绪、计划与秘密</summary><div><article><span>${label(9)}</span><p data-capture="10">$10</p></article><article><span>${label(10)}</span><p data-capture="11">$11</p></article><div class="sta-layered-mobile-destinations"><article><span>${label(11)}</span><p data-capture="12">$12</p></article><article><span>${label(12)}</span><p data-capture="13">$13</p></article><article><span>${label(13)}</span><p data-capture="14">$14</p></article></div><article class="is-secret"><span>${label(14)}</span><p data-capture="15">$15</p></article></div></details><footer><span>${skin.overline}</span><b>${skin.number}</b></footer></section>`;
        const style = `<style ${marker}>.sta-layered-mobile{display:none}@media(max-width:700px){html,body{min-height:0!important;background:transparent!important}body{display:block!important;overflow:visible!important}.status{width:100%!important;min-height:0!important;aspect-ratio:auto!important;background:transparent!important;border:0!important;filter:none!important}.status[open]>.canvas{display:none!important}.status[open]>.sta-layered-mobile{display:block!important}.status>summary[aria-label="展开或收起状态栏"]{right:12px!important;top:12px!important}.sta-layered-mobile{position:relative;overflow:hidden;padding:13px;color:var(--sta-deep);border:1px solid color-mix(in srgb,var(--sta-deep) 58%,transparent);border-radius:18px;background:radial-gradient(circle at 92% 10%,color-mix(in srgb,var(--sta-accent) 18%,transparent),transparent 28%),linear-gradient(150deg,var(--sta-paper),var(--sta-wash));box-shadow:0 10px 28px color-mix(in srgb,var(--sta-deep) 22%,transparent);font:400 clamp(14px,3.8vw,17px)/1.62 "Noto Serif SC","Source Han Serif SC","Songti SC",serif}.sta-layered-mobile:before{content:"";position:absolute;right:-42px;bottom:138px;width:138px;height:138px;border:22px solid color-mix(in srgb,var(--sta-accent) 10%,transparent);border-radius:50%;pointer-events:none}.sta-layered-mobile>*{position:relative}.sta-layered-mobile-head{display:grid;grid-template-columns:102px minmax(0,1fr);gap:13px;align-items:end;padding:2px 38px 14px 2px;border-bottom:4px double var(--sta-deep)}.sta-layered-mobile-avatar{height:140px;overflow:hidden;border:7px solid color-mix(in srgb,var(--sta-paper) 88%,white);border-radius:52px 52px 12px 12px;background:var(--sta-wash);box-shadow:0 7px 18px color-mix(in srgb,var(--sta-deep) 25%,transparent),inset 0 0 0 1px var(--sta-deep)}.sta-layered-mobile-avatar img{display:block;width:100%;height:100%;object-fit:cover;object-position:center 18%}.sta-layered-mobile-head small{display:block;color:var(--sta-accent);font:700 9px/1.35 Georgia,serif;letter-spacing:.12em}.sta-layered-mobile-head h1{margin:5px 0 4px;color:var(--sta-deep);font-size:clamp(25px,7.6vw,34px);font-weight:650;line-height:1.1;letter-spacing:.06em}.sta-layered-mobile-head p{margin:0;color:color-mix(in srgb,var(--sta-deep) 72%,white);font-size:.78em;letter-spacing:.08em}.sta-layered-mobile-meters{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin-top:11px}.sta-layered-mobile-meters article{min-width:0;padding:10px 11px;border-top:4px solid var(--sta-accent);background:color-mix(in srgb,var(--sta-paper) 82%,white)}.sta-layered-mobile span{display:block;color:var(--sta-accent);font-size:.78em;font-weight:650;letter-spacing:.11em}.sta-layered-mobile-meters strong{display:block;margin-top:4px;color:var(--sta-deep);font-size:1.1em;font-weight:600;line-height:1.4;overflow-wrap:anywhere}.sta-layered-mobile-primary,.sta-layered-mobile-body{display:grid;gap:8px;margin-top:9px}.sta-layered-mobile-primary article,.sta-layered-mobile-body article,.sta-layered-mobile-more article{min-width:0;padding:11px 12px;border-left:4px solid var(--sta-accent);background:color-mix(in srgb,var(--sta-paper) 84%,white);box-shadow:0 4px 12px color-mix(in srgb,var(--sta-deep) 9%,transparent)}.sta-layered-mobile-primary article:nth-child(even),.sta-layered-mobile-body article:nth-child(even){border-left-color:var(--sta-deep);background:color-mix(in srgb,var(--sta-wash) 72%,white)}.sta-layered-mobile-primary .is-letter{border:1px dashed var(--sta-accent);border-left:6px double var(--sta-accent);background:linear-gradient(145deg,color-mix(in srgb,var(--sta-paper) 90%,white),color-mix(in srgb,var(--sta-wash) 76%,white))}.sta-layered-mobile p{margin:5px 0 0;line-height:1.65;white-space:normal;overflow-wrap:anywhere}.sta-layered-mobile-body{grid-template-columns:minmax(0,1fr)}.sta-layered-mobile-more{margin-top:10px;border:1px solid color-mix(in srgb,var(--sta-deep) 30%,transparent);border-radius:13px;background:color-mix(in srgb,var(--sta-wash) 72%,white)}.sta-layered-mobile-more>summary{padding:12px 13px;cursor:pointer;list-style:none;color:var(--sta-deep);font-weight:600;letter-spacing:.08em}.sta-layered-mobile-more>summary:after{content:"＋";float:right;color:var(--sta-accent)}.sta-layered-mobile-more[open]>summary:after{content:"－"}.sta-layered-mobile-more>div{display:grid;gap:8px;padding:0 9px 9px}.sta-layered-mobile-destinations{display:grid;grid-template-columns:minmax(0,1fr);gap:7px}.sta-layered-mobile-more .is-secret{color:color-mix(in srgb,var(--sta-paper) 88%,white);border-left-color:var(--sta-accent);background:var(--sta-deep)}.sta-layered-mobile-more .is-secret span{color:color-mix(in srgb,var(--sta-accent) 62%,white)}.sta-layered-mobile footer{display:flex;align-items:center;justify-content:space-between;margin-top:10px;padding:9px 5px 0;border-top:1px dashed color-mix(in srgb,var(--sta-deep) 42%,transparent)}.sta-layered-mobile footer span{color:var(--sta-deep);font:700 8px/1.3 Georgia,serif}.sta-layered-mobile footer b{display:grid;width:36px;height:36px;place-items:center;border:2px solid var(--sta-accent);border-radius:50%;color:var(--sta-accent);background:color-mix(in srgb,var(--sta-paper) 80%,white);font:700 13px/1 Georgia,serif}}@media(max-width:390px){.sta-layered-mobile{padding:11px}.sta-layered-mobile-head{grid-template-columns:90px minmax(0,1fr);gap:10px;padding-right:35px}.sta-layered-mobile-avatar{height:124px}.sta-layered-mobile-meters{grid-template-columns:minmax(0,1fr)}.sta-layered-mobile-head h1{letter-spacing:.03em}}</style>`;
        const runtime = `<script>(function(){var source=document.querySelector('.status .canvas img.avatar,.status .canvas .avatar img,.status .canvas img');var target=document.querySelector('.sta-layered-mobile-avatar img');if(source&&target){target.src=source.currentSrc||source.src;target.alt='当前角色头像';}})();</script>`;
        let next = replacement.replace('<div class="compact">', `${mobile}<div class="compact">`);
        next = /<\/head>/i.test(next) ? next.replace(/<\/head>/i, `${style}</head>`) : `${style}${next}`;
...
export function applyStatusBeautyMobileTypography(regexScript, rule) {
    ...
    const style = `<style ${marker}>@media(max-width:700px){[data-sta-typography-label]{font-size:13px!important;font-weight:600!important;line-height:1.45!important;letter-spacing:.06em!important}[data-sta-typography-value][data-sta-kind="text"],[data-sta-typography-value][data-sta-kind="long"]{font-family:"Noto Sans SC","Microsoft YaHei",sans-serif!important;font-size:15px!important;font-weight:400!important;line-height:1.65!important;letter-spacing:0!important;white-space:normal!important;overflow-wrap:anywhere!important}[data-sta-typography-value][data-sta-kind="progress"]{font-weight:600!important}}</style>`;
