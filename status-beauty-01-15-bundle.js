...
export function applyStatusBeautyMobileLayout(regexScript, rule) {
    if (rule?.structure === 'moon-collage') {
        ...
        const mobile = `<section class="sta-moon-mobile" ${marker} aria-label="月下蝶影手机布局"><header class="sta-moon-mobile-head"><div class="sta-moon-mobile-photo"><img alt="当前角色头像"></div><div><small>STATUS ATELIER · MOON 03</small><h1 data-design-title>${title}</h1><p>月色、蝶影与此刻未说出口的话</p></div></header><div class="sta-moon-mobile-meters"><article><span>${label(0)}</span><strong data-capture="1">$1</strong></article><article><span>${label(1)}</span><strong data-capture="2">$2</strong></article></div><div class="sta-moon-mobile-records"><article><span>${label(2)}</span><p data-capture="3">$3</p></article><article><span>${label(3)}</span><p data-capture="4">$4</p></article><article><span>${label(4)}</span><p data-capture="5">$5</p></article><article><span>${label(5)}</span><p data-capture="6">$6</p></article></div><footer class="sta-moon-mobile-notes"><article><span>${label(6)}</span><p data-capture="7">$7</p></article><article><span>${label(7)}</span><p data-capture="8">$8</p></article></footer></section>`;
        ...
        const runtime = `<script>(function(){var source=document.querySelector('.moon-art .art-photo');var target=document.querySelector('.sta-moon-mobile-photo img');if(source&&target){target.src=source.currentSrc||source.src;target.alt=source.alt||'当前角色头像';}})();</script>`;
    ...
    if (['beauty-clock-travel-11', 'beauty-flower-reader-12', 'beauty-olive-ticket-13', 'beauty-cat-rabbit-14', 'beauty-rabbit-track-15'].includes(rule?.structure)) {
        ...
        const runtime = `<script>(function(){var source=document.querySelector('.status .canvas img.avatar,.status .canvas .avatar img,.status .canvas img');var target=document.querySelector('.sta-layered-mobile-avatar img');if(source&&target){target.src=source.currentSrc||source.src;target.alt=source.alt||'当前角色头像';}})();</script>`;
        let next = replacement.replace('<div class="compact">', `${mobile}<div class="compact">`);
        next = /<\/head>/i.test(next) ? next.replace(/<\/head>/i, `${style}</head>`) : `${style}${next}`;
        next = /<\/body>/i.test(next) ? next.replace(/<\/body>/i, `${runtime}</body>`) : `${next}${runtime}`;
        return { ...regexScript, replaceString: next };
