import { ORBIT_DEFAULTS, normalizeOrbit, buildOrbitPage, buildOrbitGreetingFields } from './opening-lunar-orbit.js';
const $ = id => document.getElementById(id);
const frame = document.querySelector('iframe');
let data = structuredClone(ORBIT_DEFAULTS), selected = data.entries[0].id, current = 'home';
try { const saved = localStorage.getItem('status-atelier-lunar-orbit-draft-v1'); if (saved) { data = normalizeOrbit(JSON.parse(saved)); selected = data.entries[0]?.id; } } catch { $('editor-status').textContent = '本地草稿读取失败，当前显示初始模板。'; }
...
function render() {
  ...
  const adapter = `<script>window.getChatMessages=async()=>parent.orbitPreviewRead();window.setChatMessages=async(rows,options)=>parent.orbitPreviewSwitch(rows,options);
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(title){editable(title,()=>parent.orbitQuickEdit('title'),'点击修改作品名称');const hint=document.createElement('small');hint.textContent='点击标题修改名称';hint.style.cssText='font:9px sans-serif;opacity:.6;display:block;margin:-12px 0 17px';title.after(hint);}
    document.querySelectorAll('[data-image-slot]').forEach(slot=>editable(slot,()=>parent.orbitQuickEdit('image',slot.dataset.imageSlot),'添加或更换本条配图'));
  });<\/script>`;
  frame.srcdoc = buildOrbitPage(data, current).replace('<body>', '<body>' + adapter);
...
window.orbitPreviewRead = () => [{ swipes: ['home', ...data.entries.map(item => item.id)].map(id => `<main data-star-page="orbit-${id}"></main>`) }];
window.orbitPreviewSwitch = async rows => {
...
$('save').onclick = () => { try { localStorage.setItem('status-atelier-lunar-orbit-draft-v1', JSON.stringify(data)); $('editor-status').textContent = '草稿已保存在本机浏览器'; } catch { $('editor-status').textContent = '本地保存失败，请下载可编辑配置保存。'; } };
...
$('export-draft').onclick = () => download('循月而行-可编辑配置.json', data);
$('export-pages').onclick = () => download('循月而行-开场白字段包.json', buildOrbitGreetingFields(data));
let quickTarget;
window.orbitQuickEdit = (kind, id) => {
  quickTarget = {kind, id};
