const STATUS_BEAUTY_05_09_STYLESHEET_URL = new URL('./status-beauty-05-09.css?v=0.11.7', import.meta.url).href;
...
(function(){
  ...
  var textSelector='p,span,strong,small,em,b,li,dt,dd,label,figcaption,h1,h2,h3,h4,h5,h6,button,summary,th,td,[data-capture],[data-value],[data-label]';var originalTextStyles=new WeakMap();
  ...
  function syncMobileReadability(){var scale=mobileScale();root.style.setProperty('--sta-readable-font',(scale<1?Math.ceil(8/scale):8)+'px');root.querySelectorAll(textSelector).forEach(function(node){var state=originalTextStyles.get(node);if(!state){state={value:node.style.getPropertyValue('font-size'),priority:node.style.getPropertyPriority('font-size'),fontSize:parseFloat(getComputedStyle(node).fontSize)||0};originalTextStyles.set(node,state)}if(state.fontSize>0&&state.fontSize*scale<8){var maxBoost=state.fontSize<=9?1.4:state.fontSize<=12?1.25:1.12;node.style.setProperty('font-size',Math.min(8/scale,state.fontSize*maxBoost)+'px','important')}else if(state.value){node.style.setProperty('font-size',state.value,state.priority)}else{node.style.removeProperty('font-size')}})}
  ...
  function syncLayout(){syncMobileReadability();requestAnimationFrame(syncHostFrameHeight);}syncLayout();window.addEventListener('resize',syncLayout);
  ...
  requestAnimationFrame(syncHostFrameHeight);if(window.ResizeObserver)new ResizeObserver(function(){requestAnimationFrame(syncHostFrameHeight);}).observe(root);
})();
</script></body></html>`;
}
