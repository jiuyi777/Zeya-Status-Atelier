const STATUS_BEAUTY_05_09_STYLESHEET_URL = new URL('./status-beauty-05-09.css?v=0.11.25', import.meta.url).href;
...
<article class="status-card design-06">
  ...
  <div class="expanded-content">
    ...
    <section class="card-table"><div class="table-moment"><article><span data-label="0">时间</span><strong data-value="0"></strong></article><article><span data-label="1">位置</span><strong data-value="1"></strong></article></div><small class="swipe-hint">← 左右滑动查看牌面 →</small>
      <div class="status-hand" aria-label="可左右滑动的角色状态牌"><article class="playing-card affection"><span>♥ <i data-label="2">情愫</i></span><strong data-value="2"></strong><small data-value="3"></small></article><article class="playing-card body"><span>♠ <i data-label="4">身体情况</i></span><strong data-value="4"></strong><small data-value="5"></small></article><article class="playing-card hands"><span>♣ <i data-label="6">双手动作</i></span><strong data-value="6"></strong><small data-value="7"></small></article><article class="playing-card mood-art"><span>♦ <i data-label="8">心情</i></span><strong data-value="8"></strong><small data-value="9"></small></article></div>
...
(function(){
  ...
  root.querySelectorAll('[data-design-title]').forEach(function(node){node.textContent=config.title;});root.querySelectorAll('[data-label]').forEach(function(node){node.textContent=config.labels[Number(node.dataset.label)]||'';});root.querySelectorAll('[data-value]').forEach(function(node){var index=Number(node.dataset.value);node.textContent=values[index]||'X';node.dataset.staKind=config.kinds[index]||'text';});
  ...
  var avatar=config.photoUrl;try{if(config.avatarSource==='character'&&!avatar&&character&&character.avatar){avatar=ctx&&typeof ctx.getThumbnailUrl==='function'?ctx.getThumbnailUrl('avatar',character.avatar):'/thumbnail?type=avatar&file='+encodeURIComponent(character.avatar);}}catch(error){avatar=config.photoUrl;}if(config.avatarSource==='none')avatar='';
