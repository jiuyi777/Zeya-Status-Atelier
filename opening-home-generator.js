...
export function buildOpeningHomeBlock(input) {
    ...
    const lines = [
        ...
        ...data.entries.map(entry => `[Opening|${escapeValue(entry.number)}|${escapeValue(entry.title)}|${escapeValue(entry.summary)}|${entry.target}|${escapeValue(entry.worldlineId)}]`),
        '</opening_home>',
    ];
    return lines.join('\n');
}
...
(function(script){
  ...
  function oldOpening(entry){return entry.length>=8;}function openingTarget(entry,index){return Math.max(1,Number(entry[oldOpening(entry)?5:4])||index+2);}function openingWorldline(entry){return entry[oldOpening(entry)?6:5]||'';}function worldlineEntries(line){return decodeEntries(line.length>=5?line[4]:line[3]);}
