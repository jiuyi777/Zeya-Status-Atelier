...
- 新增前选择黄铜别针、红格绑带、银色票夹或旧报纸相框款式；新地点自动寻找空位，不会一直叠在中央。
- 自行输入地点名称、可选短介绍和 http / https 图片 URL。
- 设定当前位置、已走过、可前往、已阻断、未探明状态。
- 调整旋转、大小与层级。
- 从画布工具栏快速复制、删除地点，并支持撤销删除。

画布底图使用用户参考素材清理出的木桌线索簿 WebP，不再由 CSS 模拟桌面与纸张。四种地点附件也直接使用用户提供的透明抠图：黄铜别针、红白格绑带、银色票夹与旧报纸木框。草稿写入浏览器本机存储。点击“生成正则”会将底图和四种附件都转成 data URI 内嵌进可导入 JSON；最终 `replaceString` 是可离线携带素材的 fenced HTML 完整文档，只包含成品地图、可选短介绍与返回动作，不包含属性面板、拖动提示、删除按钮或模板名称工具栏。
