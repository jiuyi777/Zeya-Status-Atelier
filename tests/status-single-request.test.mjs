...
function harness(reply, quietOnly = false) {
    const calls = { raw: 0, quiet: 0, applied: 0 };
    const node = () => ({ textContent: '', dataset: {}, hidden: false, disabled: false });
    const status = node();
    const stored = { structure: 'chat', statusRecentRecommendations: [] };
    ...
    const sandbox = {
        ...
        statusAiSingleCandidates: () => [candidate], resolveStatusIdeaIntent: () => ({}),
        applyStatusAiRecommendation: () => { calls.applied++; }, resolvedStatusInput: () => input,
        field: () => null,
        renderStatusSchema() {}, renderModalStatusSchema() {}, statusRecommendationKey: () => 'chat',
