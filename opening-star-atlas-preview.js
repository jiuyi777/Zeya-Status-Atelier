import { STAR_DEFAULTS, normalizeStar, buildStarPage, buildStarGreetingFields } from './opening-star-atlas.js';
const $ = id => document.getElementById(id);
const frame = document.querySelector('iframe');
let data = structuredClone(STAR_DEFAULTS), selected = data.entries[0].id, current = 'home';
try { const saved = localStorage.getItem('status-atelier-star-atlas-draft-v1'); if (saved) { data = normalizeStar(JSON.parse(saved)); selected = data.entries[0]?.id; } } catch { $('editor-status').textContent = '本地草稿读取失败，当前显示初始模板。'; }
...
function render() {
  ...
  const adapter = `<script>window.getChatMessages=async()=>parent.starPreviewRead();window.setChatMessages=async(rows,options)=>parent.starPreviewSwitch(rows,options);<\/script>`;
  frame.srcdoc = buildStarPage(data, current).replace('<body>', '<body>' + adapter);
...
window.starPreviewRead = () => [{ swipes: ['home', ...data.entries.map(item => item.id)].map(id => `<main data-star-page="${id}"></main>`) }];
window.starPreviewSwitch = async rows => {
...
for (const key of ['title', 'subtitle', 'author', 'intro', 'accent', 'ink', 'fontSize', 'fontStyle']) {
...
$('save').onclick = () => { try { localStorage.setItem('status-atelier-star-atlas-draft-v1', JSON.stringify(data)); $('editor-status').textContent = '草稿已保存在本机浏览器'; } catch { $('editor-status').textContent = '本地保存失败，请下载可编辑配置保存。'; } };
...
$('export-draft').onclick = () => download('星芒云笺-可编辑配置.json', data);
$('export-pages').onclick = () => download('星芒云笺-开场白字段包.json', buildStarGreetingFields(data));
updateSelection();
