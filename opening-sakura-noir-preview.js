import { NOIR_DEFAULTS, normalizeNoir, buildNoirPage, buildNoirGreetingFields } from './opening-sakura-noir.js';
const $ = id => document.getElementById(id);
const frame = document.querySelector('iframe');
let data = structuredClone(NOIR_DEFAULTS), selected = data.entries[0].id, current = 'home';
try { const saved = localStorage.getItem('status-atelier-sakura-noir-draft-v1'); if (saved) { data = normalizeNoir(JSON.parse(saved)); selected = data.entries[0]?.id; } } catch { $('editor-status').textContent = '本地草稿读取失败，当前显示初始模板。'; }
...
function render() {
  ...
  const adapter = `<script>window.getChatMessages=async()=>parent.noirPreviewRead();window.setChatMessages=async(rows,options)=>parent.noirPreviewSwitch(rows,options);
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(title){editable(title,()=>parent.noirQuickEdit('title'),'点击修改作品名称');const hint=document.createElement('small');hint.textContent='点击标题修改名称';hint.style.cssText='font:9px sans-serif;opacity:.6;display:block;margin:-12px 0 17px';title.after(hint);}
    document.querySelectorAll('[data-image-slot]').forEach(slot=>editable(slot,()=>parent.noirQuickEdit('image',slot.dataset.imageSlot),'添加或更换本条配图'));
  });<\/script>`;
  frame.srcdoc = buildNoirPage(data, current).replace('<body>', '<body>' + adapter);
...
window.noirPreviewRead = () => [{ swipes: ['home', ...data.entries.map(item => item.id)].map(id => `<main data-star-page="noir-${id}"></main>`) }];
window.noirPreviewSwitch = async rows => {
...
$('save').onclick = () => { try { localStorage.setItem('status-atelier-sakura-noir-draft-v1', JSON.stringify(data)); $('editor-status').textContent = '草稿已保存在本机浏览器'; } catch { $('editor-status').textContent = '本地保存失败，请下载可编辑配置保存。'; } };
...
$('export-draft').onclick = () => download('樱色夜刊-可编辑配置.json', data);
$('export-pages').onclick = () => download('樱色夜刊-开场白字段包.json', buildNoirGreetingFields(data));
let quickTarget;
window.noirQuickEdit = (kind, id) => {
  quickTarget = {kind, id};
